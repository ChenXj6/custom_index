...
// 新增：平均值（由你自己设置）
const averageValue = ref(40) // 这里设置为110，你可以根据需要修改
...
function createChart() {
  ...
  // 数据
  const data = [35, 20, 8, 11, 26, 30, 20];
  ...
  let option = {
    xAxis: {
      type: 'category',
      data: ['11-29', '11-30', '11-31', '12-01', '12-02', '12-03', '12-04'],
      axisLabel: {
        color: '#999',
        fontSize: 12,
        margin: 10
    ...
    yAxis: {
      ...
      max: function(value) {
        // 确保Y轴最大值能显示平均值
        return Math.max(value.max, maxTop.value, averageValue.value);
      },
      axisLabel: {
        color: '#999',
        fontSize: 12
    ...
    tooltip: {
      ...
      formatter: function(params) {
        ...
        return `${data.name}<br/>学员数量: ${data.value}`;
      }
    },
