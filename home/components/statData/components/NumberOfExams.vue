...
// 新增：平均值（由你自己设置）
const averageValue = ref(21) // 这里设置为110，你可以根据需要修改
...
function createChart() {
  ...
  let option = {
    xAxis: {
      type: 'category',
      data: ['一总队', '山东边检站', '天津边检站', '北京边检站', '河南边检站'],
      axisLabel: {
        color: '#999',
        fontSize: 12,
        margin: 10,
    ...
    yAxis: {
      ...
      max: function(value) {
        // 确保Y轴最大值能显示平均值
        return Math.max(value.max, maxTop.value, averageValue.value, dataMax);
      },
      axisLabel: {
        color: '#999',
        fontSize: 12
          ...
          data: [
            {
              ...
              label: {
                ...
                formatter: averageValue.value + '场次',
                color: '#FF9900',
                fontSize: 12,
                fontWeight: '400',
                // 关键修改：将标签放在线条上方
